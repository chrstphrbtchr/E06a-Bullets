...
### CURRENTLY DOES NOT WORK, THOUGH NOT SURE WHY
...
class bullet_enemy(arcade.Sprite):
    def __init__(self, position, velocity, damage):
        super().__init__("assets/bullet_enemy.png", 0.3)
        (self.center_x, self.center_y) = position
        (self.dx, self.dy) = velocity
        self.damage = damage
    def update(self):
        '''
        Moves the BAD bullet
        '''
        self.center_x += self.dx
        self.center_y += self.dy
...
class Window(arcade.Window):
    ...
    def update(self, delta_time):
        ...
        for e in self.enemy_list:
            # check for collision (dmg. has a list of every time e & bullets interact)
        ...
        for Enemy in self.enemy_list:
            #arcade.check_for_collision_with_list(q,self.player)
            if random.randint(1,20) <= 20:
                self.enemy_bullet_list.draw()
                badbullet = bullet_enemy
                badbullet.position = Enemy.position
                badbullet.angle = -90
                badbullet.change_y = -3
                #self.enemy_bullet_list.append(badbullet)???
                x = Enemy.center_x
                y = Enemy.center_y -10
                badbullet = bullet_enemy((x,y), (0,10), BULLET_DAMAGE)
                random.randint(1,20)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        if button == arcade.MOUSE_BUTTON_LEFT:
            #fire a bullet
            # Below is added code that creates bullet upon click
